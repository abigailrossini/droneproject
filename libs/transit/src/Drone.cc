...
Drone::Drone(JsonObject& obj, DroneObserver* obv)
...
void Drone::getNextDelivery() {
  if (model && model->scheduledDeliveries.size() > 0) {
    ...
    if (package) {
      ...
      Notify("Heading to " + package->getName());
    }
  }
}
void Drone::update(double dt) {
  ...
  if (toPackage) {
    ...
    if (toPackage->isCompleted()) {
      ...
      toPackage = nullptr;
      pickedUp = true;
      Notify("Picked up " + package->getName());
    }
    ...
    if (toFinalDestination->isCompleted()) {
      Notify("Finished delivery of " + package->getName());
